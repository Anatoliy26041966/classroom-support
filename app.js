...
// --- Функція генерації динамічної шапки ---
function renderHeader() {
  ...
  const navItems = [
    ...
    { name: 'Батькам', href: 'parents.html' },
    { name: 'Вчителям', href: 'teachers.html' },
    { name: 'Google Meet', href: 'meet.html' },
    { name: 'Статус', href: 'status.html' }
  ];
  ...
  const navLinksHtml = navItems.map(item => {
    ...
    return `<a href="${item.href}" class="${styles} px-3 py-2 rounded-lg">${item.name}</a>`;
  }).join('');
  ...
  header.innerHTML = `
    <div class="max-w-6xl mx-auto px-4 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
      ...
      <nav class="flex items-center gap-2 text-xs font-semibold flex-wrap">
        ${navLinksHtml}
        <a href="index.html#support" class="bg-amber-500 hover:bg-amber-400 text-slate-900 px-3 py-2 rounded-lg transition ml-1">Техпідтримка</a>
      </nav>
    </div>
  `;
}
